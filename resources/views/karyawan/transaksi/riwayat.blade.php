...
<h3 class="mb-4 fw-bold text-primary">📜 Riwayat Transaksi</h3>
...
<div class="card-body">
    ...
    <!-- ===== FORM FILTER ===== -->
    <form method="GET" action="{{ route('karyawan.transaksi.riwayat') }}" class="row g-3 mb-3" id="filter-form">
        ...
        <div class="col-12 col-md-6 d-flex flex-column flex-md-row gap-2 align-items-start align-items-md-end">
            <button class="btn btn-primary w-100 w-md-auto">🔍 Filter</button>
            <a href="{{ route('karyawan.transaksi.riwayat') }}" class="btn btn-secondary w-100 w-md-auto">Reset</a>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card shadow-sm h-100" style="background:linear-gradient(135deg,#db2777,#ec4899);">
                <div class="card-body text-white text-center">
                    ...
                    <div class="fw-bold fs-6">Rp {{ number_format($totalPenjualan,0,',','.') }}</div>
                </div>
            </div>
        </div>
    ...
    {{--
        ✅ table-mobile-card  → aktifkan mode card di mobile
        Setiap <td> wajib punya data-label="..." agar label muncul otomatis
    --}}
            ...
            @foreach($transactions as $trx)
                <tr>
                    ...
                    <td data-label="Status" class="text-center">
                        ...
                        @else
                            <span class="badge bg-secondary rounded-pill px-3">{{ ucfirst($trx->status) }}</span>
                        @endif
                    </td>
                    ...
                    <td data-label="Aksi" class="text-center">
                        <div class="d-flex gap-1 justify-content-center flex-wrap">
                            ...
                            <a href="{{ route('karyawan.transaksi.show', $trx->id) }}"
                               class="btn btn-sm btn-primary rounded-pill px-3">
                                Detail
                            </a>

                            @if(!$trx->retur && $trx->status === 'completed')
                                <button type="button"
                                        class="btn btn-sm btn-outline-warning rounded-pill px-3"
                                        onclick="konfirmasiRetur({{ $trx->id }})">
                                    🔄 Retur
                                </button>

                                <form id="retur-form-{{ $trx->id }}"
                                      method="POST"
                                      action="{{ route('karyawan.retur.store', $trx->id) }}"
                                      style="display:none;">
                                    @csrf
                                </form>
                            @endif
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius:18px; overflow:hidden;">
            <div class="modal-header text-white border-0"
                ...
                <h5 class="modal-title fw-bold">🔄 Konfirmasi Retur</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-center py-4">
                <div style="font-size:48px;" class="mb-3">🔄</div>
                <p class="mb-1">Yakin ingin meretur transaksi ini?</p>
                <p class="text-muted small">Stok barang akan dikembalikan ke gudang.</p>
            </div>
...
<script>
    ...
    /* ===== KONFIRMASI RETUR ===== */
    let returFormId = null;

    function konfirmasiRetur(id){
        returFormId = id;
        new bootstrap.Modal(document.getElementById('returModal')).show();
    }

    document.getElementById('btn-konfirmasi-retur').addEventListener('click', function(){
        if(returFormId){
            document.getElementById('retur-form-' + returFormId).submit();
        }
    });
